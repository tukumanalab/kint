...
| **総労働時間** (`total_working_hours`) | 各日における実仕事時間の合計。<br>算定式: $\sum (\text{check\_out} - \text{check\_in})$ (時間換算、小数点以下2位)。※`check_out` が未打刻の日（打刻漏れ）は計算から除外。 |
...
**ヘッダー定義**:
```csv
日付,表示名,氏名,シフト開始時刻,シフト終了時刻,出勤時間,退勤時間,実労働時間(h),時間外労働時間(h),遅刻判定,早退判定,勤怠ステータス,打刻ソース,修正理由
```

**出力サンプル**:
```csv
2026-05-01,yamada,山田 太郎,09:00,18:00,08:55,18:15,8.25,0.25,-,否,正常,webusb_nfc,
2026-05-01,sasaki,佐々木 美咲,09:00,18:00,09:15,18:00,7.75,0.00,是,否,遅刻,web_user_id,寝坊のため
2026-05-02,yamada,山田 太郎,09:00,18:00,08:59,,0.00,0.00,-,-,打刻漏れ,webusb_nfc,
```
...
**ヘッダー定義**:
```csv
対象月,ユーザーID,表示名,氏名,所定労働日数,実出勤日数,総労働時間(h),時間外労働時間(h),遅刻回数,早退回数,欠勤日数,打刻エラー日数
```

**出力サンプル**:
```csv
2026-05,usr_01,yamada,山田 太郎,21,20,162.50,12.25,1,0,1,0
2026-05,usr_02,sasaki,佐々木 美咲,21,18,140.00,4.50,3,1,3,1
```
