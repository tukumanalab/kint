...
- アカウントID（`users.id`）はメールアドレスと同一のものが自動設定され、これをログイン識別子として使用します。
- アカウントID は 3〜254 文字、利用可能文字は英数字と `_.@+-`。
- 既定の管理者ユーザーは未作成時のみ `account ID: manager / password: manager123` で作成されます。
- 一度作成された後は、パスワードを変更しても起動時に上書きされない。

### ユーザー登録時の必須項目

管理者によるユーザー登録時は、以下をすべて必須で入力する（アカウントIDは入力不要であり、入力されたメールアドレスが自動的にアカウントIDとして使用されます）。
...
- アカウントIDおよびメールアドレス重複時: `ACCOUNT_ID_CONFLICT` または `EMAIL_CONFLICT`
