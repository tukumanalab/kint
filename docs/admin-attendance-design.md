...
- **UC-01: 月次勤怠サマリーの参照**
  - 管理者は、指定年月のワーカー全員の出勤状況（実出勤日数、総労働時間、時間外労働時間、遅刻、早退、欠勤回数等）を一覧でサマライズして閲覧できる。
...
- **UC-04: 本人による自分の月次サマリーの参照（マイページ拡張）**
  - 一般ユーザーも、マイページから自分の指定月の労働サマリーを閲覧できる。
...
| **実労働日数** (`working_days`) | `attendances` のうち、該当ユーザーの `work_date` が当該月内にあり、`check_in` が存在するユニークな日付の日数。 |
| **有給/公休日数** (`holiday_days`) | 会社の休日や有給、シフトがそもそもない日数（所定労働日以外）。 |
| **総労働時間** (`total_working_hours`) | 各日における実仕事時間の合計。<br>算定式: $\sum (\text{check\_out} - \text{check\_in})$ (時間換算、画面表示やCSV出力では `[h]:mm` 形式で表示/出力)。※`check_out` が未打刻の日（打刻漏れ）は計算から除外。 |
| **総時間外労働時間** (`total_overtime_hours`) | 所定の勤務時間を超過して働いた時間。以下のいずれかのルールで計算する。<br>① **日次シフト超過ルール**（デフォルト）: 各勤務日において、`check_out` が `Shift.end_time` を超過した場合の累計時間。 <br>② **1日8時間超ルール**: 各勤務日の労働時間が 8時間を超えた分の累計時間。 |
| **遅刻回数** (`late_count`) | `shifts` が存在する勤務日において、以下を満たす回数。<br>$\text{Attendance.check\_in} > \text{Shift.start\_time}$ (かつ、その差が1分以上) |
| **早退回数** (`early_leave_count`) | `shifts` が存在する勤務日において、以下を満たす回数。<br>$\text{Attendance.check\_out} < \text{Shift.end_time}$ (かつ、その差が1分以上) |
    ...
    AttendanceMonthlySummary:
      ...
      properties:
        ...
        total_working_hours:
          type: number
          format: float
          description: 当月の総労働時間 (h)
          example: 162.50
        total_overtime_hours:
          type: number
          format: float
          description: 当月の総時間外労働時間 (h)
          example: 12.25
        late_count:
          type: integer
          description: 遅刻回数
    ...
    DailyAttendanceDetail:
      ...
      properties:
        ...
        working_hours:
          ...
          format: float
          nullable: true
          description: 1日の稼働時間(h)
        overtime_hours:
          type: number
          format: float
          nullable: true
          description: 1日の時間外労働時間(h)
        status:
          type: string
          enum: [normal, late, early_leave, late_and_early, absence, incomplete, off_duty]
          description: 日次の勤怠状態
          example: "normal"
        source:
          type: string
...
**ヘッダー定義**:
```csv
日付,表示名,氏名,シフト開始時刻,シフト終了時刻,出勤時間,退勤時間,勤務出勤,勤務退勤,実労働時間,時間外労働時間,遅刻判定,早退判定,勤怠ステータス,打刻ソース,修正理由
```

**出力サンプル**:
```csv
2026-05-01,yamada,山田 太郎,09:00,18:00,08:55,18:15,09:00,18:00,8:15,0:15,-,否,正常,webusb_nfc,
2026-05-01,sasaki,佐々木 美咲,09:00,18:00,09:15,18:00,09:15,18:00,7:45,0:00,是,否,遅刻,web_user_id,寝坊のため
2026-05-02,yamada,山田 太郎,09:00,18:00,08:59,,08:59,,0:00,0:00,-,-,打刻漏れ,webusb_nfc,
```
...
**ヘッダー定義**:
```csv
対象月,ユーザーID,表示名,氏名,所定労働日数,実出勤日数,総労働時間,時間外労働時間,遅刻回数,早退回数,欠勤日数,打刻エラー日数
```

**出力サンプル**:
```csv
2026-05,usr_01,yamada,山田 太郎,21,20,162:30,12:15,1,0,1,0
2026-05,usr_02,sasaki,佐々木 美咲,21,18,140:00,4:30,3,1,3,1
```
