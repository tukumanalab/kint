...
- ADR-007: Server と NFC打刻クライアントは Python 実行環境を分離する。
  - 根拠: OS差異（Linux/Windows）と依存ライブラリ差異（FastAPI系/nfcpy・PySide6）により、同一環境運用では競合と再現性低下が起きるため。

## 8. トレードオフ整理
- 現時点の推奨構成はモジュラモノリス。
- 将来的に外部 API 負荷が増えた場合は、Calendar 同期を別ワーカーサービスへ分離する。
- Python 環境は分離運用になるため、依存更新・セキュリティパッチ適用は Server 側と Desktop 側で個別管理が必要になる。
