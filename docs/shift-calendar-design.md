...
- **同期対象期間**: 同期実行日（`today = date.today()`）を基準とした**前後2週間（14日間）**（`today - 14日` 〜 `today + 14日`）。対象範囲外のイベントはスキップします。
...
- **削除ロジック**: 
  1. 今回iCalから取得できた対象期間（同期実行日を基準とした前後2週間）の全 `UID` をリスト化。
  2. DB上に存在する「前後2週間のシフト」のうち、取得した `UID` リストに含まれないものは、「元のカレンダー上で削除されたシフト」と判断し、DB上から削除します。（※対象期間外の過去・未来のシフトは削除対象から除外されます）
...
3. **APIエンドポイント作成**:
   - `POST /api/v1/shifts/sync`: バックグラウンド同期タスクを登録して非同期受理（202）。
   - `POST /api/v1/shifts/sync/now`: 即時ブロッキング同期を実行し、結果（200）またはエラー（400）を返却。
...
手動同期（非同期 `POST /api/v1/shifts/sync` および即時同期 `POST /api/v1/shifts/sync/now`）は引き続き利用可能。
...
### 8-1. 概要
管理者がシステム設定画面から、登録された iCal 同期 URL からシフトデータを直ちに手動同期し、取得できたデータ件数やエラーの有無をリアルタイムで確認できる機能。

### 8-2. API エンドポイント `POST /api/v1/shifts/sync/now`
- **権限**: 管理者 (`admin`) 専用。
- **処理**: ブロッキング処理として `CalendarSyncService.sync()` を `await` で実行し、結果を応答する。
- **レスポンス (成功時: 200 OK)**:
  - `success`: `true`
  - `message`: `"同期が成功しました"`
  - `stats`: `{"inserted": X, "updated": Y, "deleted": Z, "skipped": W}`
- **レスポンス (失敗時: 400 Bad Request)**:
  - iCal URL未設定やHTTP取得失敗、パースエラーの際、`KintBadRequestError` を発生させ、エラー詳細（例: `code="CALENDAR_SYNC_FAILED"`, `message="SHIFT_ICAL_URL が設定されていません"`）を返却する。
